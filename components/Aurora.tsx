...
// Base Colors matched to Utsav '26 Ananta Theme
const vec3 BLACK = vec3(0.01, 0.0, 0.03); 
const vec3 PURPLE = vec3(147.0, 51.0, 234.0) / 255.0; 
const vec3 SILVER = vec3(192.0, 192.0, 192.0) / 255.0; 
...
vec3 background_color(vec2 uv) {
  ...
  float m = uv.y - y;
  col += mix(PURPLE, BLACK, smoothstep(0.0, 1.0, abs(m)));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec3 col = vec3(0.0);
  vec3 b = lineGradientCount > 0 ? BLACK : background_color(baseUv);
...
export default function Aurora({
  linesGradient = ["#9333EA", "#C0C0C0", "#1A0B2E"], 
  ...
  useEffect(() => {
    ...
    // STRICT VIEWPORT LOCK FOR THREEJS DOM ELEMENT
    ...
    const setSize = () => {
      // Force dimensions to match the window exactly
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      if (parallax) {
        ...
        const offsetY = -(y - centerY) / window.innerHeight;
  ...
  return (
    // FIX: strict viewport locking (inset-0, w-screen, h-screen) so it never scrolls
    <div
      ref={containerRef}
      className="fixed inset-0 w-screen h-screen -z-50 bg-[#04010a] pointer-events-none overflow-hidden"
    />
  );
}